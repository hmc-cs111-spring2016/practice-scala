...
object ListManipulationExercise02 {
  ...
  def sumOfTwo(l1: List[Int], l2: List[Int]): List[Int] = {
    l1.padTo(l2 length, 0).zip(l2.padTo(l1 length, 0)) map {case (x,y) => x + y}
  }
...
//  
    val (youngins,elders) = persons partition (_.age < 18)
    val youngNames = youngins.sortBy(_ age).map(_ firstName)
    val oldNames = elders.sortBy(_ age).map(_ firstName)
