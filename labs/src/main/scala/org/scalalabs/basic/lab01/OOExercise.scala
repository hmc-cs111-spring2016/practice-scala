...
class Euro(val euro:Int, val cents:Int=0) extends Currency("EUR") with Ordered[Euro]{
  lazy val inCents = 100 * euro + cents
  
  def +(addMe:Euro) = {
    val centsSum = inCents + addMe.inCents
    Euro.fromCents(centsSum)
 }
  
  def *(factor:Int) = {
    val centsProduct = inCents * factor
    Euro.fromCents(centsProduct)
  }
  ...
  def compare(compareTo:Euro) = inCents - compareTo.inCents
}
