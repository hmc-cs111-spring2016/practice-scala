...
class Euro(val euro: Int, val cents: Int = 0) extends Currency("EUR"){
  ...
  def +(otherEuro: Euro): Euro = {
    ...
    // This is the code I initially wrote :P  Goes to show how foreign Scala objects feel at the moment.
    /* val totalCents = this.inCents + otherEuro.inCents
     * new Euro(totalCents/100, totalCents%100)
  	 */
